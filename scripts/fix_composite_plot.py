...
def create_fixed_composite_plot(all_campaigns, output_dir):
    ...
    line_best = ax2.plot(trial_range, mean_best, 'gray', linewidth=4, alpha=0.3, label=f'Mean Best {obj1_name}', zorder=1)
    lines += line_best
    ...
    line_gp_r2 = ax2_r2.plot(np.array(trial_range)[valid_indices], np.array(gp_r2_values)[valid_indices], 
                           'red', label='GP R²', marker='o', linewidth=2, zorder=3)
    lines += line_gp_r2
    ...
    line_ax_r2 = ax2_r2.plot(np.array(trial_range)[valid_indices], np.array(ax_cv_r2_values)[valid_indices], 
                           'orange', label='Ax CV R²', marker='d', linewidth=2, linestyle='--', zorder=3)
    lines += line_ax_r2
    ...
    line_tau = ax2_tau.plot(np.array(trial_range)[valid_indices], np.array(rank_tau_values)[valid_indices], 
                          'green', label='Rank τ', marker='s', linewidth=2, zorder=3)
    lines += line_tau
    ...
    line_loo = ax2_loo.plot(np.array(trial_range)[valid_indices], np.array(loo_values)[valid_indices], 
                          'blue', label='LOO NLL', marker='^', linewidth=2, zorder=3)
    lines += line_loo
    ...
    line_gp_int = ax2_gp_int.plot(np.array(trial_range)[valid_indices], np.array(gp_interval_values)[valid_indices], 
                                'purple', label='GP Interval Score', marker='v', linewidth=2, linestyle=':', zorder=3)
    lines += line_gp_int
    ...
    line_ax_int = ax2_ax_int.plot(np.array(trial_range)[valid_indices], np.array(ax_cv_interval_values)[valid_indices], 
                                'magenta', label='Ax CV Interval Score', marker='*', linewidth=2, linestyle='-.', zorder=3)
    lines += line_ax_int
    ...
    line_imp_std = ax2_imp_std.plot(np.array(trial_range)[valid_indices], np.array(importance_std_values)[valid_indices], 
                                  'pink', label='Importance Std', marker='p', linewidth=2, linestyle='-.', zorder=3)
    lines += line_imp_std
