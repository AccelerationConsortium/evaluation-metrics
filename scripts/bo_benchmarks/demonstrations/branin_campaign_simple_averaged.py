...
def main():
    ...
    for trial_idx in trial_indices:
        ...
        if trial_data:
            ...
            averaged_metric = {
                ...
                'ax_cv_r2': safe_mean([m['ax_cv_r2'] for m in trial_data if m['ax_cv_r2'] is not None]),
                'ax_cv_interval_score': safe_mean([m['ax_cv_interval_score'] for m in trial_data if m['ax_cv_interval_score'] is not None])
            }
    ...
    if averaged_metrics:
        ...
        line_gp_r2 = ax.plot(trial_nums, gp_r2_values, 'r-', label='GP R² (avg)', marker='o', linewidth=2, zorder=3)
        lines = line_gp_r2
        
        # Plot Ax CV R² if available (also on left axis)
        if ax_cv_r2_values:
            line_ax_r2 = ax.plot(ax_cv_trial_nums, ax_cv_r2_values, 'orange', label='Ax CV R² (avg)', marker='d', 
        ...
        ax.set_title(f"Averaged Evaluation Metrics vs Trial ({len(all_campaign_metrics)} Campaigns)")
