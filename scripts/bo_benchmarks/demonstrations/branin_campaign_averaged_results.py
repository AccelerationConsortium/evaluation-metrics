...
def calculate_iteration_metrics(df, objective_name, trial_index):
    ...
    y_train_tensor = torch.tensor(y_train, dtype=torch.float64).unsqueeze(-1)  # Add output dimension
...
def calculate_ax_cross_validation_metrics(cv_client, trial_index):
    ...
    try:
        ...
        assert fig["data"][1].error_y is not None, "Uncertainty data not available from Ax cross-validation"
        ...
        print(f"Trial {trial_index}: Ax CV R² = {ax_cv_r2:.4f}, Ax CV Interval Score = {ax_cv_interval_score:.4f}")
...
def run_single_campaign(seed, num_trials=50):
    ...
    for i in range(num_trials):
        ...
        if trial_index >= 2:  # Calculate metrics starting from trial 3 (index 2)
            ...
            if metrics:
                # Calculate Ax cross-validation metrics
                ax_cv_r2, ax_cv_interval_score = calculate_ax_cross_validation_metrics(cv_client, trial_index)
    ...
    for trial_idx in common_trial_indices:
        ...
        if trial_metrics:
            ...
            averaged_metric = {
                ...
                'ax_cv_interval_score': safe_mean([m['ax_cv_interval_score'] for m in trial_metrics])
            }
...
if __name__ == "__main__":
    ...
    if averaged_metrics:
        ...
        ax_cv_r2_values = [m['ax_cv_r2'] for m in averaged_metrics if (m['ax_cv_r2'] is not None and np.isfinite(m['ax_cv_r2']))]
        interval_score_values = [m['interval_score'] for m in averaged_metrics if m['interval_score'] is not None]
        ax_cv_interval_score_values = [m['ax_cv_interval_score'] for m in averaged_metrics if (m['ax_cv_interval_score'] is not None and np.isfinite(m['ax_cv_interval_score']))]
        
        ax_cv_trial_nums = [m['trial'] for m in averaged_metrics if (m['ax_cv_r2'] is not None and np.isfinite(m['ax_cv_r2']))]
        interval_trial_nums = [m['trial'] for m in averaged_metrics if m['interval_score'] is not None]
        ax_cv_interval_trial_nums = [m['trial'] for m in averaged_metrics if (m['ax_cv_interval_score'] is not None and np.isfinite(m['ax_cv_interval_score']))]
        ...
        line_best = ax.plot(trial_nums, best_so_far_values, color='gray', linewidth=6, alpha=0.3, 
                           label='Best so far (avg)', linestyle='-', zorder=1)
        
        # Plot R² values on right axis 1
        line_gp_r2 = ax_r2.plot(trial_nums, gp_r2_values, 'r-', label='GP R² (avg)', marker='o', linewidth=2, zorder=3)
        lines = line_best + line_gp_r2
        
        # Plot Ax CV R² if available (also on right axis 1)
        if ax_cv_r2_values:
            line_ax_r2 = ax_r2.plot(ax_cv_trial_nums, ax_cv_r2_values, 'orange', label='Ax CV R² (avg)', marker='d', 
        ...
        line_tau = ax_tau.plot(trial_nums, rank_tau_values, 'g-', label='Rank τ (avg)', marker='s', linewidth=2, zorder=3)
        lines += line_tau
        ax_tau.set_ylim(-1, 1)
        
        # Plot LOO NLL on right axis 3 (auto range)
        line_loo = ax_loo.plot(trial_nums, loo_values, 'b-', label='LOO NLL (avg)', marker='^', linewidth=2, zorder=3)
        lines += line_loo
        
        # Plot GP interval score on right axis 4 (auto range)
        if interval_score_values:
            line_gp_int = ax_gp_int.plot(interval_trial_nums, interval_score_values, 'purple', label='GP Interval Score (avg)', 
        ...
        if ax_cv_interval_score_values:
            line_ax_int = ax_ax_int.plot(ax_cv_interval_trial_nums, ax_cv_interval_score_values, 'magenta', 
                                  label='Ax CV Interval Score (avg)', marker='*', linewidth=2, linestyle='-.', zorder=3)
            lines += line_ax_int
        
        # Plot importance std dev on right axis 6 (auto range)
        line_imp_std = ax_imp_std.plot(trial_nums, imp_std_values, 'pink', 
                                  label='Importance Std (avg)', marker='p', linewidth=2, linestyle='-.', zorder=3)
        lines += line_imp_std
        ...
        legend = ax.legend(lines, labels, loc='upper left', framealpha=0.8, fancybox=False, shadow=False)
